                ...
                <div class="card-body">
                    <h4 class="mb-5 header-title"><i class="fas fa-list"></i> <?= $title ?>
                        <div class="float-right mr-1">
                            <a href="" class="btn btn-block btn-success btn-sm" data-toggle="modal" data-target="#importData"><i class="fa fa-file-import"></i> Import</a>
                        </div>
                    </h4>
                    <?= $this->session->flashdata('message') ?>
                    <form style="margin: 20px 0;" action="<?= base_url() . 'karyawan/daftar_siswa'; ?>" method="post">
                        <div class="form-row">
                            <div class="form-group col-lg-3">
                                <label>Tambah siswa</label>
                                <a href="<?= base_url('karyawan/tambah_siswa'); ?>" class="btn btn-block btn-info"><i class="fa fa-plus-circle"></i> Pendaftaran siswa</a>
                            </div>
                            <div class="form-group col-lg-3">
                        ...
                        <tbody>
                            ...
                            foreach ($siswa as $d) : ?>
                                ...
                                <tr>
                                    ...
                                    <?php if ($d['point'] == 100) : ?>
                                        <td><span style="border-radius:25px" class="btn btn-sm btn-rounded btn-success" disabled><?= $d['point'] ?></span></td>
                                    <?php elseif ($d['point'] >= 70 || $d['point'] >= 99) : ?>
                                        <td><span style="border-radius:25px" class="btn btn-sm btn-rounded btn-primary" disabled><?= $d['point'] ?></span></td>
                                    <?php elseif ($d['point'] >= 50) : ?>
                                        <td><span style="border-radius:25px" class="btn btn-sm btn-rounded btn-warning" disabled><?= $d['point'] ?></span></td>
                                    <?php elseif ($d['point'] >= 30 || $d['point'] >= 1) : ?>
                                        <td><span style="border-radius:25px" class="btn btn-sm btn-rounded btn-danger" disabled><?= $d['point'] ?></span></td>
                                    <?php elseif ($d['point'] == 0) : ?>
                                        <td><span style="border-radius:25px" class="btn btn-sm btn-rounded btn-secondary" disabled><?= $d['point'] ?></span></td>
                                    <?php endif ?>
                                    <td>
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            ...
                            <a href="<?= base_url('hapus/hapus_siswa/karyawan?id=') ?><?= $d['id'] ?>" class="btn btn-danger"><i class="fa fa-trash"></i> Hapus</a>
                        </div>
